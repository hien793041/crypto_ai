...
app.run(function ($rootScope) {
	...
	$("#Main_Stock_Search").autocomplete({
		minLength: 1
	});

	$rootScope.Complete_Main_Stock = function () {
		$rootScope.availableTags = [];
		var input =
		{
			Name: $rootScope.MainStock
		};
		$.ajax({
			type: 'POST',
			...
			success: function (data) {
				for (var i = 0; i < data.length; i++) {
					...
					$rootScope.availableTags.push(obj);
				}
			},
		...
		$("#Main_Stock_Search").autocomplete({
			source: $rootScope.availableTags,
			select: function (event, ui) {
				$rootScope.MainStockId = ui.item.id;
				angular.element(document.getElementById('kt_content')).scope().Stock_Search(ui.item.value);
			}
		});
